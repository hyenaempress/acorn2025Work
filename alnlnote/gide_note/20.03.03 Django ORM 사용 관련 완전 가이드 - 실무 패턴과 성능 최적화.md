...
> 이 문서는 **20.03.02 Django MariaDB 연결 및 DB 포인트 추가 정리**에 이어지는 고급 내용입니다.

## 목차
1. [기본 설정 복습 (20.03.02 요약)](#기본-설정-복습-20-03-02-요약)
2. [Django ORM vs Raw SQL 선택 기준](#django-orm-vs-raw-sql-선택-기준)
3. [Django ORM 핵심 원칙과 패턴](#django-orm-핵심-원칙과-패턴)
4. [모델 설정 및 관계 정의](#모델-설정-및-관계-정의)
5. [ORM 뷰 함수 구현](#orm-뷰-함수-구현)
6. [ORM 전용 템플릿 구성](#orm-전용-템플릿-구성)
...
## 기본 설정 복습 (20.03.02 요약)

### 📋 선행 조건 체크리스트
ORM을 사용하기 전에 다음 기본 설정이 완료되어야 합니다:
...
class Board(models.Model):
    ...
    class Meta:
        managed = False  # 🚨 ORM 사용 시 True로 변경 필요!
        db_table = 'board'
...
class Jikwon(models.Model):
    ...
    busernum = models.IntegerField()  # 🚨 ORM 사용 시 ForeignKey로 변경 필요!
...
class Gogek(models.Model):
    ...
    gogekdamsano = models.ForeignKey('Jikwon', models.DO_NOTHING, 
                                   db_column='gogekdamsano', blank=True, null=True)
...
#### 6. 모델 복사 및 수정 🔥 중요 단계!
**강사님:** *"aa.py의 내용을 myapp/models.py에 복사하여 사용합니다."*

1. `aa.py`의 내용을 `myapp/models.py`에 복사
2. ORM 사용을 위해 다음 수정 필요:
   - `managed = False` → `managed = True`
   - `IntegerField()` → `ForeignKey()` (관계 필드)
   - `on_delete` 옵션 추가

managed = False → managed = True
→ Django가 이 테이블을 관리하게 할지 여부를 지정하는 건데, ORM에서 마이그레이션으로 생성·수정하려면 True로 바꿔야 해요.

IntegerField() → ForeignKey()
→ 관계를 맺는 필드(다른 테이블 PK를 참조하는 컬럼)는 IntegerField 대신 ForeignKey로 변경하고,
on_delete=models.CASCADE 같은 옵션을 꼭 붙여야 해요. (DO_NOTHING 유지할 수도 있지만, 보통 CASCADE 권장)
...
**강사님 조언:** *"마이그레이션은 언제하는거에요? 테이블을 만들고 마이그레이션 하고 마이그레이트 하는거에요."*

### ✅ ORM 사용 준비 완료 체크
- [ ] Django 프로젝트 생성됨
- [ ] MySQL/MariaDB 연결 설정 완료
- [ ] `inspectdb`로 모델 파일 생성됨
- [ ] `models.py`에 모델 복사됨
- [ ] 마이그레이션 실행 완료
- [ ] 앱이 `INSTALLED_APPS`에 등록됨

**이제 Django ORM을 사용할 준비가 완료되었습니다!** 🎉

#### 8. URL 설정 (mainapp/urls.py)
```python
# mainapp/urls.py
...
urlpatterns = [
    ...
    # ORM 관련 URL은 아래에서 추가 예정
]
```

#### 9. 기본 뷰 함수 (myapp/views.py)
```python
# myapp/views.py - 기본 버전 (Raw SQL 사용)
...
def dbshowFunc(request):
    """Raw SQL을 사용한 직원 정보 조회"""
    sql = """
    SELECT j.jikwonno, j.jikwonname, b.busername, j.jikwonjik, j.jikwonpay
    FROM jikwon j
    ...
    return render(request, 'dbshow.html', {'rows': rows})
```
