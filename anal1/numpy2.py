...
print(f"\na: \n{a}")  # 2차원 배열
print(f"a[0, 0]: {a[0, 0]}")
print(f"a[0, 1]: {a[0, 1]}")  # 첫 번째 행, 두 번째 열
print(f"a[1 : 2]: {a[1 : 2]}")      # 두 번째 행 (2차원 유지)
print(a[1:, 0:2])   # 두 번째 행부터 첫 번째 열과 두 번째 열까지
print(a[0], '', a[1])  # 첫 번째 행과 두 번째 행
#차원 떨어뜨리기
# 첫 번째 행과 첫 번째 요소
print(f"a[0][0]: {a[0][0]}")  # 첫 번째 행의 첫 번째 요소
print(f"a[0][1]: {a[0][1]}")  # 첫 번째 행의 두 번째 요소
print(f"a[0].shape: {a[0].shape}")  # (3,) - 1차원 배열

aa= np.array((1,2,3))
print(aa)
bb = aa[1:3]  # 슬라이싱 (1부터 2까지) 서브 배열 
print(bb, '' , bb[0])  # (2,) - 1차원 배열]

cc = aa[1:3].copy()  # 슬라이싱 후 복사
 
 
print(cc)
cc[0] = 55  # 복사본 수정
print(f"cc: {cc}")  # (2,) - 1차원 배열
print(f"aa: {aa}")  # 원본 배열은 변경되지 않음
print(f"aa[1:3]: {aa[1:3]}")  # (2,) - 1차원 배열

r1 = a[1, :] # 두 번째 행 전체 1차원 배열이 됨 
r2 = a[1:2, :]  # 두 번째 행을 2차원 배열로 유지

c1 = a[:, 1] #전체 행 1열 슬라이싱 
c2 = a[:, 1:2]  # 전체 행에서 두 번째 열을 2차원 배열로 유지


print(c1, c1.shape)  # (2,) - 1차원 배열
print(c2, c2.shape)  # (2, 1) - 2차원 배열

print()
print(a)
bool_idx = (a >= 5)  # 불리언 인덱싱
print(f"불리언 인덱싱: {bool_idx}")  # 불리언 배열
print(a[bool_idx])  # 불리언 인덱싱을 사용하여 5 이상인 요소 선택
