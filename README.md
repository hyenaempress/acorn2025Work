...
- [0. 학습 로드맵 & 개요](#00-학습-로드맵--개요)
- [1. NumPy (기초→심화)](#01-numpy-기초심화)
- [2. 수치 변환·정규화](#02-수치-변환정규화)
- [3. Pandas (기초)](#03-pandas-기초)
- [4. 파일·웹 융합](#04-파일웹-융합)
- [5. 웹 크롤링 기초](#p5-웹-크롤링-기초)
- [6. 데이터 전처리 & 활용](#06-데이터-전처리--활용)
- [7. XML & JSON 데이터 처리](#07-xml--json-데이터-처리)
- [08. 데이터 시각화](#8-데이터-시각화)
- [09. 데이터베이스 연동](#9-데이터베이스-연동)
...
- [20.00.05 Feature(피처변수) 복습겸 정리](alnlnote/gide_note/20.00.05%20Feature(피처변수)%20복습겸%20정리.md) ✅ **NEW**
...
- [22.00.04 Feature(변수) 기초 개념](alnlnote/gide_note/22.00.04%20Feature(변수)%20기초%20개념.md) ✅ **NEW**
- [22.00.05 Feature(피처변수) 완전 정리 - 전체 문서 횡단 분석](alnlnote/gide_note/22.00.05%20Feature(피처변수)%20완전%20정리%20-%20전체%20문서%20횡단%20분석.md) ✅ **NEW**
...
#### 25. ANOVA (분산분석) 심화✅
- ☐ **25.01** 일원분산분석 (One-way ANOVA) 심화✅
- ☐ **25.02** 이원분산분석 (Two-way ANOVA) 심화✅
- ☐ **25.03** 사후검정 (Post-hoc Tests) 완전 가이드🟨
...
- ☐ **22.11.05** 다중선형회귀: 여러 독립변수로 확장 🟨 **집필 중**
- ☐ **22.11.06** 회귀진단: 가정 위반 해결책 🟨 **집필 중**
- ☐ **22.11.07** 정규화 회귀: Ridge, Lasso 등 고급 기법 🟨 **집필 중**
- ☐ **22.11.08** 로지스틱 회귀: 분류 문제로 확장 🟨 **집필 중**
...
- **통계학 심화가 필요하다면**: 22.06-22.11번 섹션에서 T검정, ANOVA, 회귀분석까지 완전 마스터
...
10. **통계학 심화**: `22.06~22.11` (T검정 + ANOVA + 회귀분석) ✅ **진행 중**
...
- **통계학 기초**: `22.00~22.05` (DIKW 피라미드 + 기술통계 + 추론통계 + 카이제곱)
- **통계학 심화**: `22.06~22.11` (T검정 + ANOVA + 회귀분석) ✅ **진행 중**
- **통계학 전체**: `23~26` (카이제곱 심화 + t검정 심화 + ANOVA 심화 + 회귀분석 심화) **계획**
...
| **통계학 전문 연구** ✅ **NEW** | `22.06~22.11` (T검정 + ANOVA + 회귀분석) |
| **AI 챗봇 시스템** | `22 시리즈 + 39.01 + 40.01 + 41.01` (통계 + ML 기초 + RAG + LangChain) **계획** |
| **AI 추천 시스템** | `22 시리즈 + 39.01 + 39.02 + 41.03` (통계 + ML 기초 + 신경망 + 파인튜닝) **계획** |
| **AI 웹 애플리케이션** | `20 시리즈 + 22 시리즈 + 40.01 + 40.03` (Django + 통계 + RAG + LLM 서빙) **계획** |
| **기업 AI 전략 수립** | `22 시리즈 + 42.01 + 42.02 + 42.03` (통계 + 기획 + 비용분석 + 전략) **계획** |
| **고급 알고리즘 융합** | `11.10 + 14.02 + 18.01` (유니버스 실무 응용) |

### 🎯 **새로운 학습 경로 (통계학 완전체 포함)**
...
- **완전체 구축**: T검정, ANOVA, 회귀분석까지 통계학 핵심 영역 커버 ✅ **진행 중**
- **통계학 전문가**: 카이제곱·t검정·ANOVA·회귀분석 심화로 완전체 달성 **계획**
...
- **2025-08-23**: 통계학 심화 과정 진행 중 (22.06~22.11 시리즈: T검정 + ANOVA + 회귀분석 핵심 완료, 22.11.05~22.11.08 집필 중)
- **2025-08-23**: 통계학 전체 완성 로드맵 추가 (23~26 섹션: 카이제곱·t검정·ANOVA·회귀분석 심화 계획)
- **2025-08-23**: Feature(피처변수) 개념 통합 문서 추가 (22.00.04, 22.00.05, 20.00.05)
...
**Note**: 문서 내 '뷰티풀 스눕' → **BeautifulSoup**로 통일, Django/장고 표기도 혼용 대신 **Django**로 통일했습니다. 섹션 번호 충돌(20.02 중복 등) 제거·정렬했고, AI/ML 섹션은 체크박스로 상태를 명확히 표시합니다. **통계학 섹션(12~13)**은 기초에서 심화까지 체계적으로 구성되어 있으며, **22.11.05~22.11.08** 섹션이 집필 중이고 **23~26** 섹션으로 완전한 통계학 전문가 과정이 계획되어 있습니다!
